...
const CONFIG = {
    ...
    gravity: 25, flySpeed: 20, actionDuration: 200, bedrockDepth: -10,
    playerRadius: 0.3
};
...
const inventoryItems = ['grass', 'dirt', 'stone', 'wood', 'leaves', 'sand', 'glass', 'brick', 'planks', 'diamond', 'dandelion', 'poppy', 'pickaxe', 'sword'];
let hotbarSlots = ['grass', 'dirt', 'stone', 'wood', 'planks', 'glass', 'dandelion', 'poppy', 'pickaxe'];
...
const tData = {
    ...
    diamond: loadTex('block/diamond_block.png'),
    bedrock: loadTex('block/bedrock.png'),
    dandelion: loadTex('block/dandelion.png'),
    poppy: loadTex('block/poppy.png'),
    pig: loadTex('entity/pig/pig.png'),
    cow: loadTex('entity/cow/cow.png'),
    villager: loadTex('entity/villager/villager.png'),
    player: loadTex('entity/steve.png')
};

const materialsMap = {
    dirt: new THREE.MeshLambertMaterial({ map: tData.dirt }),
    grass: [
        new THREE.MeshLambertMaterial({ map: tData.grass_side }),
        new THREE.MeshLambertMaterial({ map: tData.grass_side }),
        new THREE.MeshLambertMaterial({ map: tData.grass_top }),
        new THREE.MeshLambertMaterial({ map: tData.dirt }),
        new THREE.MeshLambertMaterial({ map: tData.grass_side }),
        new THREE.MeshLambertMaterial({ map: tData.grass_side })
    ],
    stone: new THREE.MeshLambertMaterial({ map: tData.stone }),
    wood: [
        new THREE.MeshLambertMaterial({ map: tData.wood }),
        new THREE.MeshLambertMaterial({ map: tData.wood }),
        new THREE.MeshLambertMaterial({ map: tData.wood_top }),
        new THREE.MeshLambertMaterial({ map: tData.wood_top }),
        new THREE.MeshLambertMaterial({ map: tData.wood }),
        new THREE.MeshLambertMaterial({ map: tData.wood })
    ],
    leaves: new THREE.MeshLambertMaterial({ map: tData.leaves, transparent: true, alphaTest: 0.5 }),
    bedrock: new THREE.MeshLambertMaterial({ map: tData.bedrock }),
    sand: new THREE.MeshLambertMaterial({ map: tData.sand }),
    glass: new THREE.MeshLambertMaterial({ map: tData.glass, transparent: true, opacity: 0.6 }),
    brick: new THREE.MeshLambertMaterial({ map: tData.brick }),
    planks: new THREE.MeshLambertMaterial({ map: tData.planks }),
    diamond: new THREE.MeshLambertMaterial({ map: tData.diamond }),
    dandelion: new THREE.MeshLambertMaterial({ map: tData.dandelion, transparent: true, alphaTest: 0.5, side: THREE.DoubleSide }),
    poppy: new THREE.MeshLambertMaterial({ map: tData.poppy, transparent: true, alphaTest: 0.5, side: THREE.DoubleSide })
};
...
const playSound = (name) => { if(sounds[name]) { sounds[name].currentTime = 0; sounds[name].play().catch(()=>{}); } };
...
const plantGeo = new THREE.PlaneGeometry(0.8, 0.8);
...
function setupInstancedMeshes() {
    allTypes.forEach(t => {
        ...
        // Instanced mesh with enough capacity
...
function noise2D(x, z) {
    const s = 0.05;
    return Math.sin(x * s) * Math.cos(z * s) * 4 + Math.sin(x * s * 0.5) * 6;
}
...
function generateWorld() {
    ...
    if(Math.random() > 0.3) villageCenters.push({x: 10, z: 10});
        ...
        for (let cz = -CONFIG.worldSize; cz < CONFIG.worldSize; cz++) {
            for (let x = 0; x < CONFIG.chunkSize; x++) {
                for (let z = 0; z < CONFIG.chunkSize; z++) {
                    ...
                    // Flatten around villages
                    let vDist = 999;
                    villageCenters.forEach(v => {
                        const d = Math.sqrt((wx-v.x)**2 + (wz-v.z)**2);
                        if(d < vDist) vDist = d;
                    });
                    
                    let rawH = noise2D(wx, wz);
                    if (vDist < 15) rawH = rawH * (vDist/15); // Flatten out
                    ...
                    // Trees & Plants
                    if (vDist >= 15 && Math.random() < 0.01) {
                        buildTree(wx, h, wz);
                    } else if (vDist >= 15 && Math.random() < 0.05) {
                        world.set(getBlockKey(wx, h+1, wz), { type: Math.random() > 0.5 ? 'dandelion' : 'poppy' });
                    }
                }
            }
        }
...
const dummyPlant1 = new THREE.Object3D();
const dummyPlant2 = new THREE.Object3D();
...
function updateInstances() {
    ...
    for (const [key, data] of world.entries()) {
        ...
        if (data.type === 'dandelion' || data.type === 'poppy') {
            // Render 2 crossed planes for plants
            dummyPlant1.position.set(x, y - 0.5, z);
            dummyPlant1.rotation.y = Math.PI / 4;
            dummyPlant1.updateMatrix();
            if (counts[data.type] < CONFIG.maxInstances) blockMeshes[data.type].setMatrixAt(counts[data.type]++, dummyPlant1.matrix);
            
            dummyPlant2.position.set(x, y - 0.5, z);
            dummyPlant2.rotation.y = -Math.PI / 4;
            dummyPlant2.updateMatrix();
...
function createMobModel(tex, isQuad) {
...
function spawnMob(type, x, y, z) {
    ...
    const model = createMobModel(tData[type], isQuad);
    model.g.position.set(x, y, z);
    scene.add(model.g);
    entities.push({
        ...model, type, dir: new THREE.Vector3(Math.random()-0.5, 0, Math.random()-0.5).normalize(),
        speed: 1 + Math.random(), timer: Math.random()*5, isQuad
    });
}
function spawnVillager(x, y, z) { spawnMob('villager', x, y, z); }

for(let i=0; i<15; i++) spawnMob(Math.random()>0.5 ? 'pig' : 'cow', (Math.random()-0.5)*40, 20, (Math.random()-0.5)*40);
...
const pModel = createMobModel(tData.player, false); pModel.g.position.y = -1.5; pModel.g.rotation.y = Math.PI; player.add(pModel.g);
...
function renderUI() {
    if (hotbarEl) {
        ...
        hotbarSlots.forEach((t, i) => {
            ...
            if(t !== '') s.style.backgroundImage = `url(${ASSET_URL}textures/block/${t === 'wood'?'oak_log':(t === 'planks'?'oak_planks':(t === 'grass'?'grass_block_side':t))}.png)`;
            s.innerText = i+1;
...
if (invGrid) {
    inventoryItems.forEach(t => {
        ...
        s.style.backgroundImage = `url(${ASSET_URL}textures/block/${t === 'wood'?'oak_log':(t === 'planks'?'oak_planks':(t === 'grass'?'grass_block_side':t))}.png)`;
...
function animate() {
    ...
    if (state.gameStarted) {
        ...
        } else { pModel.legs.forEach(l => l.rotation.x = 0); }
        ...
        entities.forEach(e => {
            ...
            if (e.dir.lengthSq() > 0.1) {
                ...
                else { e.legs[0].rotation.x=sLeg; e.legs[1].rotation.x=-sLeg; }
            ...
            else { e.g.position.y = Math.floor(e.g.position.y - 0.1 + 0.5) + (e.isQuad?0.5:0.8); } // Adjust for height
        });
    }
